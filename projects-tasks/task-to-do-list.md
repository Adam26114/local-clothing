...
- [ ] Add Better Auth persistence schema support in Convex (custom adapter data model).
- [ ] Implement custom Better Auth adapter on Convex queries/mutations.
- [ ] Wire functional `/api/auth/[...all]` handlers.
- [ ] Migrate cookie-shim login/register to Better Auth flows.
- [ ] Keep superadmin allowlist behavior via `SUPERADMIN_EMAILS`.
- [ ] Remove legacy shim session implementation after migration.

Acceptance criteria:
- [ ] `zweaungnaing.info@gmail.com` auto-promotes to admin on first authenticated sync.
- [ ] Non-admin users are blocked from `/admin/*`.
- [ ] Better Auth sessions govern admin authorization.
...
- [ ] Stage 2 Better Auth + Convex custom adapter implementation.
- [ ] Orders module migration from mock data.
- [ ] Users/settings admin migration from mock data.
- [ ] Cart/checkout/account migration from mock data.
