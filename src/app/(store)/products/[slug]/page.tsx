...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const related = useMemo(() => products.filter((item) => item.slug !== params.slug).slice(0, 4), [params.slug]);
  ...
  return (
    <div className="space-y-8">
      <nav className="text-xs uppercase text-zinc-500">MEN &gt; SHIRTS &gt; {product.name}</nav>
      <section className="grid gap-8 lg:grid-cols-2">
        <div className="grid grid-cols-2 gap-2">
          {(selectedVariant.images.length > 0 ? selectedVariant.images : ['/placeholder.jpg']).map((image, index) => (
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            {lowStock ? <p className="text-xs text-orange-600">Low stock: only {currentStock} left</p> : null}
            {isOut ? <p className="text-xs text-red-600">Out of stock</p> : null}
          </div>

          <div className="flex gap-2">
            <Button className="flex-1 bg-black text-white hover:bg-zinc-800" disabled={isOut} onClick={addToCart}>
              Add to Cart
            </Button>
          ...
          <Accordion type="single" collapsible>
            ...
            <AccordionItem value="size-fit">
              ...
              <AccordionContent>Regular fit. For oversized style, choose one size up.</AccordionContent>
            </AccordionItem>
          </Accordion>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {related.map((item) => (
            <Link key={item._id} href={`/products/${item.slug}`} className="rounded border p-3 text-sm hover:bg-zinc-50">
              {item.name}
            </Link>
          ))}
        </div>
